...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:  # pylint: disable=unused-argument
    ...
    try:
        ...
        if severity == 'HIGH' and 'Unencrypted' in finding['description']:
            ...
            # Create Security Hub finding details
            security_hub_finding = {
                'SchemaVersion': '2018-10-08',
                'Id': finding['id'],
                'ProductArn': (
                    f"arn:aws:securityhub:{finding['region']}:"
                    f"{finding['accountId']}:product/aws/macie"
                ),
            ...
            security_hub.batch_import_findings([security_hub_finding])
